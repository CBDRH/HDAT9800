...
library(leaflet)
# library(rgdal) # rgdal is being retired
library(sf)
library(dplyr)

if (!require(hdat9800tutorials)) remotes::install_github('CBDRH/hdat9800')
library(hdat9800tutorials)
...
CED_polys <- sf::st_read(CED_polys_file) %>%
              sf::st_transform("+proj=longlat +datum=WGS84")
# CED_polys <- rgdal::readOGR(CED_polys_file) # rgdal is being retired
CED_polys <- CED_polys %>%
  mutate(CED_CODE_2016 = factor(paste0("CED", CED_CODE16)))

# just select one polygon with id CED101 (Banks)

CED_poly <- CED_polys %>% 
              filter(CED_CODE_2016 == "CED101")
...
median_polys <- median_polys %>% left_join(CED_median_info, 'CED_CODE_2016')
median_polys$Median_rent_weekly_diff = median_polys$Median_rent_weekly - AUST_Median_rent_weekly

# Let's find the maximum and minimum Median_rent_weekly for the CEDs we've selected
CED_max_Median_rent_weekly <- max(median_polys$Median_rent_weekly)
CED_min_Median_rent_weekly <- min(median_polys$Median_rent_weekly)
